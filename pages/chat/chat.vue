<template>
	<div class="content">
		<div class="aui-news-box">
			<div  v-if="conversations.length!=0" class="aui-news-item" v-for="(item, index) in conversations" :key="index" @tap="gotochat(item.shopId)">
				...
				<span class="aui-news-item-fr">{{item.timeStr}}</span>
			</div>
			<view class="empty" v-if="conversations.length==0">{{i18n.noMessage}}</view>
		</div>
	</div>
</template>
