...
function request (params, isGetTonken) {
	...
	var needToken = false
	if (params.url.indexOf("/api/v1/") == 0 || params.url.indexOf("/user/registerOrBindUser") == 0) {
		needToken = true
	}
	uni.request({
		...
		header: {
			...
			'Authorization': !needToken ? undefined : uni.getStorageSync('token') || wx.getStorageSync('tempToken'),
...
function loginSuccess (result, fn) {
	...
	if (!result.enabled) {
		uni.showModal({
			showCancel: false,
			title: i18n.t('index.tips'),
			content: i18n.t('index.cannotPurchaseTips'),
			cancelText: i18n.t('index.cancel'),
			confirmText: i18n.t('index.confirm')
		})
