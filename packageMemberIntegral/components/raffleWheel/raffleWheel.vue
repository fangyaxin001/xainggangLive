...
<view class="canvas-container">
  <image src="../../static/images/icon/zhuanpan@2x.png" class="pan-bg" mode="widthFix"></image>
  <view :animation="animationData" class="gb-wheel-content" :style="'width:' + (size+2) + 'rpx;height:' + size + 'rpx;'">
    ...
    <view class="canvas-list">
      <view class="canvas-item2" v-for="(item, key) in awardsConfig.awards" :key="key" :style="'transform: rotate(' + item.item2Deg + ');background-color:' + (awardsConfig.awards.length==2?item.color:'') + ';opacity:' + (awardsConfig.awards.length==2?item.opacity:awardsConfig.awards.length==3?item.opacity:'') + ';width:' + size + 'rpx;height:' + (size/2-2) + 'rpx;transform-origin:' + (size/2) + 'rpx ' + (size/2) + 'rpx'">
        ...
        <view v-if="awardsConfig.awards.length==3" class="canvas-item2-after" :style="'background-color:' + item.color + ';width:' + (size/2) + 'rpx;height:' + (size/2) + 'rpx;transform-origin: ' + (size/2) + 'rpx ' + (size/2) + 'rpx'"></view>
      </view>
    </view>
    ...
    <view class="gb-wheel-list">
      <view class="gb-wheel-item" :data-index="index" v-for="(item, key) in awardsConfig.awards" :key="key">
        <view class="gb-wheel-icontent" :style="'width:170rpx;overflow:hidden;font-size:26rpx;line-height:26rpx;padding-top:5%;transform: rotate(' + (index*turnNum) + 'turn);transform-origin: 50% ' + (size/2-1) + 'rpx'">
          ...
          <view class="item-img">
            <image v-if="item.prizeType==3" src="../../static/images/icon/youhuiq@2x.png" :data-type="i18n.coupon" style="width:102rpxmargin-top:20rpx" mode="widthFix"></image>
            <image v-if="item.prizeType==0" src="../../static/images/icon/nanguo@2x.png" :data-type="i18n.thankYourPatronage" style="width:46rpxmargin-top:20rpx" mode="widthFix"></image>
            ...
            <image v-if="item.prizeType==1" src="../../static/images/icon/jinbi@2x.png" :data-type="i18n.integral" style="width:46rpxmargin-top:20rpx" mode="widthFix"></image>
            <image v-if="item.prizeType==2" :src="item.pic" :data-type="i18n.goods" style="width:65rpxmargin-top:20rpx" mode="widthFix"></image>
          </view>
        </view>
      </view>
    </view>
  ...
  <view class="img-container" :style="'width:100%;height:' + size + 'rpx;'">
    <!-- 转盘中间的按钮 -->
    <image src="../../static/images/icon/dianjichoujiang@2x.png" :style="'width:' + (size/3.5) + 'rpx;'" mode="widthFix"></image>
    <view class="btn-text" @tap="zhuanFun" v-if="!zhuanflg">{{i18n.clickLottery}}</view>
  ...
  <!-- 装饰图片 -->
  <image class="lucky-decorate" src="../../static/images/icon/people.png" mode="widthFix"></image>
  <!-- 抽奖说明 -->
  <view class="desc">
...
export default {
  ...
  watch: {
    myProperty: function (newVal, oldVal, changedPath) {// 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
  ...
  methods: {
    ...
    reset() {
      ...
      animation.rotate(0).step(), that.runDegs = 0;
      that.setData({
    ...
    zhuanFun() {
      var that = this,
          awardsConfig = that.awardsConfig,
          runDegs = that.runDegs; //>>> 是无符号移位运算符
    ...
    getScoreLuckPrizes() {
      var params = {
        url: '/api/v1/scoreLuck/getScoreLuckPrizes',
        method: 'GET',
        data: {
          consumeType: 1
        },
